...
def is_drug_name(text: str) -> bool:
    ...
    if len(t) <= 2:
        return False
    # si contiene espacio, examina cada token
    tokens = t.split()
    for w in tokens:
        if any(w.endswith(s) for s in DRUG_SUFFIXES):
            return True
    # palabra única con patrón típico
    if any(t.endswith(s) for s in DRUG_SUFFIXES):
        return True
    return False

# Evitar tapar acrónimos genéricos (SAMUR, SUMMA…)
def is_generic_acronym(text: str) -> bool:
    t = text.strip()
    return t.isupper() and 3 <= len(t) <= 6
...
def normalize_profile(profile: dict):
    ...
    "PER", "ORG", "LOC", "GPE", "FAC"  # sin MISC; NORP fuera por defecto
    ])
...
# --- NUEVO BLOQUE: MODERACIÓN DE ANONIMIZACIÓN ORG, DETECCIÓN DE FÁRMACOS, ACRÓNIMOS, ETC. ---

PRECISION_FIRST = True

# El NER secundario sólo puede tapar ubicaciones (nunca ORG/NORP)
SECONDARY_SAFE_LABELS = {"LOC", "GPE", "FAC"}

# Lista blanca de conceptos sanitarios generales que NO deben anonimizarse 
SAFE_ORG_TERMS = {
    "urgencias", "servicio de urgencias", "médico de atención primaria",
    "medico de atencion primaria", "atención primaria", "atencion primaria",
    "samur", "summa", "uvi", "uvi móvil", "uvis", "suap", "pac"
}

# Palabras clave para confirmar que una ORG es una institución sanitaria legítima
HOSPITAL_KEYWORDS = (
    "hospital", "clínica", "clinica", "fundación", "fundacion", "instituto",
    "universitario", "unidad", "centro", "sanitario", "asistencial"
)

# Sufijos farmacológicos típicos (muy ampliables)
DRUG_SUFFIXES = (
...
def is_generic_acronym(text: str) -> bool:
    # Detecta acrónimos cortos en mayúscula tipo SAMUR, SUMMA, UVI, etc.
    t = text.strip()
    return t.isupper() and 3 <= len(t) <= 8

def looks_like_health_org(text: str) -> bool:
    """
    Consideramos institución sanitaria sólo si:
    - Contiene alguna keyword hospitalaria clara
    - Y NO es un fármaco ni un acrónimo generalista ni una palabra aislada sin sentido clínico
    """
    t = text.lower().strip()
    if any(k in t for k in HOSPITAL_KEYWORDS):
        return True
    # Opcional: permitir multi-palabra tipo "Centro de Salud Gran Vía"
    return len(text.split()) >= 3  # exige >= 3 palabras para tratarlo como institución si no tiene keyword

def anonymize_text(text: str, nlp_main, nlp_sec, tag_map: dict, regex_cfg: dict,
                   ner_labels, gazetteer, masking_strategy="tag"):
    """
    Pipeline con lógica de alta precisión:
    1) NER principal -> tapamos SOLO:
       - PER (siempre)
       - ORG SOLO si cumple criterios de institución sanitaria real (ver looks_like_health_org)
       - LOC/GPE/FAC
    2) NER secundario (solo LOC/GPE/FAC)
    3) Regex complementarias
    4) Gazetteer (si hay)
    5) Heurística "vive en..." o "reside en..." -> [LOC]
    """
    ...
    try:
        ...
        for ent in doc.ents:
            ...
            etext = ent.text
            etext_l = etext.lower().strip()

            if lab == "PER":
                spans_main.append((ent.start_char, ent.end_char, lab))
                continue

            if lab == "ORG":
                # No anonimizar si está en lista blanca
                if etext_l in SAFE_ORG_TERMS:
                    continue
                # No anonimizar si es un fármaco
                if is_drug_name(etext):
                    continue
                # No anonimizar acrónimos genéricos SAMUR/SUMMA/etc.
                if is_generic_acronym(etext) and etext_l in SAFE_ORG_TERMS:
                    continue
                # Solo anonimizar si parece institución sanitaria
                if looks_like_health_org(etext):
                    spans_main.append((ent.start_char, ent.end_char, lab))
                continue

            if lab in {"LOC", "GPE", "FAC"}:
                spans_main.append((ent.start_char, ent.end_char, lab))
                continue
    except Exception:
        pass

    # ===== 2) NER secundario (sólo ubicaciones) =====
    spans_sec = []
    if nlp_sec is not None:
        try:
            ...
            for ent in doc2.ents:
                lab2 = ent.label_
                if lab2 in SECONDARY_SAFE_LABELS:
                    spans_sec.append((ent.start_char, ent.end_char, lab2))
        except Exception:
            pass

    # Aplicar reemplazos (del final al inicio)
    ...
    # ===== 3) Regex complementarias =====
    rx_counts = {}
    def _apply(pattern, token):
        nonlocal buf, rx_counts
        if not pattern:
            return
        new, n = re.subn(pattern, token, buf, flags=re.IGNORECASE)
        if n:
            buf = new
    ...
    # ===== 5) Heurística "vive/reside/domiciliad* en ..." =====
