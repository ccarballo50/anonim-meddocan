...
COMMON_GIVEN_NAMES = {
    # Lista mínima para no ser agresivos; puedes añadir si lo necesitas
    "juan","jose","josep","maria","maría","carlos","carlas","ana","luisa","luis",
    "pedro","miguel","antonia","antonio","pablo","laura","andres","andrés","beatriz",
    "javier","jorge","manuel","isabel","cristina","francisco","fernando","rafael"
}

def is_plausible_person_name(text: str) -> bool:
    """
    Nombre plausible si:
      - Tiene ≥ 2 tokens (p. ej. "Juan Pérez")  O
      - Es un nombre muy común de pila
    Además:
      - Debe NO ser acrónimo claro
      - Debe NO parecer fármaco
    """
    if not text or not text.strip():
        return False
    t = text.strip()
    if is_generic_acronym(t):
        return False
    if is_drug_name(t):
        return False
    toks = t.split()
    if len(toks) >= 2:
        return True
    # token único: solo si es un nombre de pila muy frecuente
    return t.lower() in COMMON_GIVEN_NAMES
...
_DRUG_MATCH_NLP = None
_DRUG_MATCHER = None
_DRUG_LIST = None

def build_drug_matcher():
    """
    Crea un PhraseMatcher de spaCy con un vademécum de fármacos.
    Fuente: profiles/drug_lexicon.txt si existe; si no, usa un set mínimo embebido.
    """
    global _DRUG_MATCH_NLP, _DRUG_MATCHER, _DRUG_LIST

    if _DRUG_MATCHER is not None:
        return _DRUG_MATCH_NLP, _DRUG_MATCHER

    # 1) Cargar lexicón: archivo opcional + un mínimo embebido
    base_min = {
        "adiro", "aspirina", "paracetamol", "ibuprofeno", "clopidogrel",
        "atorvastatina", "enoxaparina", "omeprazol", "heparina", "metformina",
        "enalapril", "losartan", "amlodipino", "amoxicilina", "ceftriaxona",
        "morfina", "diazepam", "lorazepam", "omeprazol", "pantoprazol",
        "dolquine", "hidroxicloroquina", "ticagrelor", "prasugrel"
    }
    # cargar desde profiles/drug_lexicon.txt si existe
    extra = set()
    p = Path("profiles") / "drug_lexicon.txt"
    if p.exists():
        try:
            with open(p, "r", encoding="utf-8") as f:
                for line in f:
    ...
    _DRUG_LIST = sorted(base_min.union(extra))

    # 2) Tokenizador ligero (es) para construir patterns
    _DRUG_MATCH_NLP = spacy.blank("es")
    _DRUG_MATCHER = PhraseMatcher(_DRUG_MATCH_NLP.vocab, attr="LOWER")
    patterns = [_DRUG_MATCH_NLP.make_doc(term) for term in _DRUG_LIST]
    _DRUG_MATCHER.add("DRUG", patterns)

    return _DRUG_MATCH_NLP, _DRUG_MATCHER

def drug_spans_for_text(text: str):
    """
    Devuelve lista de (start_char, end_char) que cubren fármacos en 'text'
    usando PhraseMatcher (case-insensitive).
    """
    if not text or not text.strip():
        return []
    nlp_tok, matcher = build_drug_matcher()
    doc = nlp_tok(text)
    spans = []
    for _, start, end in matcher(doc):
        s = doc[start].idx
        e = doc[end - 1].idx + len(doc[end - 1])
        spans.append((s, e))
    return spans

def spans_overlap(a_start, a_end, b_start, b_end) -> bool:
    """True si dos spans [a) y [b) se solapan en caracteres."""
    return not (a_end <= b_start or b_end <= a_start)

def load_local_drug_lexicon() -> set:
    """
    Carga un vademécum local opcional:
    - profiles/drug_lexicon.txt (una entrada por línea)
    Devuelve set en minúsculas.
    """
    candidates = [
        Path("profiles") / "drug_lexicon.txt"
    ]
    terms = set()
    for p in candidates:
        try:
...
def in_drug_lexicon(text: str, lex: set) -> bool:
    if not text:
        return False
    # Coincidencia exacta por token/frase, case-insensitive
    return text.strip().lower() in lex
...
def anonymize_texts(
    ...
    with nlp_main.select_pipes(enable=["ner"]):
        # Precalcular spans de fármacos por texto (misma longitud que 'texts')
        drug_spans_list = [drug_spans_for_text(t) for t in texts]
        for doc in nlp_main.pipe(texts, batch_size=BATCH_SIZE):
            text_orig = doc.text
            # spans para PER/GPE/LOC
                # OJO: este for está dentro del for doc in pipe(...)
        idx = len(final)  # número de textos ya procesados (final crece 1 a 1)
        drug_spans = drug_spans_list[idx]
        ...
        for ent in doc.ents:
            ...
            # Si la entidad solapa un span de fármaco -> NO anonimizar
            ent_s, ent_e = ent.start_char, ent.end_char
            if any(spans_overlap(ent_s, ent_e, ds, de) for ds, de in drug_spans):
                continue

            # 1) PER: solo si parece nombre, y nunca si es acrónimo/fármaco (extra capa)
            if lab == "PER":
                if is_drug_name(etext):
                    continue
                if is_generic_acronym(etext) and low in SAFE_ORG_TERMS:
                    continue
                if is_plausible_person_name(etext):
                    spans.append((ent_s, ent_e, lab))
                continue

            # 2) Ubicaciones: descartar términos sanitarios blancos o fármacos mal etiquetados
            if lab in {"GPE", "LOC"}:
                if low in SAFE_ORG_TERMS:
                    continue
                if is_generic_acronym(etext) and low in SAFE_ORG_TERMS:
                    continue
                if is_drug_name(etext):
            ...
            # (Otros labels se ignoran por conservador)
...
def run_excel_with_profile(xlsx_path: str, profile_path: str, out_path: str = None,
    ...
    df = pd.read_excel(xlsx_path)
    drug_lexicon = load_local_drug_lexicon()
